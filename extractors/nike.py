...
class NikeExtractor(BaseExtractor):
    """
    Extractor for Nike shoes, returns a list of NikeShoe instances
    (which subclass BaseShoe).
    """
    ...
    def _get_products_from_groupings(self, stub: Optional[str], products: list) -> list:
        """Recursively fetch paginated product groupings via the Nike API stub."""
        if not stub:
            return products
    ...
    def _extract_product_data(self, product: dict) -> dict:
        """
        Pull out the 10 canonical fields for our RAW table,
        then bundle any additional bits into `extra`.
        """
        base = {
        ...
        extras = {
            ...
            'outOfStock':       any("OUT_OF_STOCK" in a for a in (product.get('featuredAttributes') or [])),
            'bestSeller':       any("BEST_SELLER"  in a for a in (product.get('featuredAttributes') or [])),
        }
        # prune falsy values
        extras = {k: v for k, v in extras.items() if v not in (None, "", False)}
        base['extra'] = json.dumps(extras, ensure_ascii=False) if extras else None

        return base

    def _process_category(self, category_path: str, config: dict) -> List[NikeShoe]:
        """Fetch, parse, and normalize one category page into NikeShoe objects."""
        logger.info(f"Processing category: {category_path}")
        start = time.time()

        # get the page & extract the Next.js JSON blob
        html = self.SESSION.get(self.SITE_BASE + category_path).text
        ...
        # pull the API stub URL for pagination
        try:
        ...
        for prod in products:
            d = self._extract_product_data(prod)
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """Apply normalization, filtering, and deduplication."""
        df['brand'] = df['brand'].str.lower().str.strip()
        df['image'] = df['image'].fillna("")

        # filter out non-shoe items
    ...
    def _run_data_quality_tests(self, df: pd.DataFrame) -> bool:
        """Basic checks: no null prices, no negative values, gender normalized."""
        ok = True
    ...
    def extract(self) -> List[NikeShoe]:
        """
        Entry point: run all categories (or a single one),
        clean + QA, and return a list of NikeShoe instances.
        """
        all_shoes: List[NikeShoe] = []

        if self.category.lower() == "all":
            for cat_path, cfg in self.CATEGORY_CONFIG.items():
                all_shoes += self._process_category(cat_path, cfg)
        else:
